...
from YOLOv6Model import YOLOv6Detector  # 从YOLOv8Model模块中导入YOLOv8Detector类，用于加载YOLOv8模型并进行目标检测
QF_Config.set_verbose(False)

cls_name = ["条形码", "二维码"]  # 定义类名列表
colors = [[random.randint(0, 255) for _ in range(3)] for _ in range(len(cls_name))]  # 为每个目标类别生成一个随机颜色

model = YOLOv6Detector()  # 创建YOLOv6Detector对象
model.load_model(abs_path("weights/best_ckpt.pt", path_type="current"))  # 加载预训练的YOLOv6模型
...
if __name__ == '__main__':  # 如果当前模块是主模块
    ...
    img_path = abs_path("test_media/t1.jpg")  # 定义图像文件的路径
