...
from YOLOv6Model import YOLOv6Detector  # 从YOLOv8Model模块中导入YOLOv8Detector类，用于进行YOLOv8物体检测
...
cls_name = ["条形码", "二维码"]  # 定义类名列表

model = YOLOv6Detector()  # 创建YOLOv8Detector对象
model.load_model(abs_path("weights/best_ckpt.pt", path_type="current"))  # 加载预训练的YOLOv8模型
colors = get_cls_color(model.names)  # 获取类别颜色

app = QtWidgets.QApplication(sys.argv)  # 创建QApplication对象
window = MainWindow()  # 创建MainWindow对象

filename = abs_path("test_media/二维码测试视频.mp4", path_type="current")  # 定义视频文件的路径
