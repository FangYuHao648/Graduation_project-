...
def get_args_parser(add_help=True, data_path='datasets/Goats/Goats.yaml', conf_file='configs/yolov6n_finetune.py',
                    name='exp', epochs=300, batch_size=8, workers=2, output_dir="runs/detect"):
    parser = argparse.ArgumentParser(description='YOLOv6 PyTorch Training', add_help=add_help)
    parser.add_argument('--data-path', default=data_path, type=str, help='path of dataset')
    parser.add_argument('--conf-file', default=conf_file, type=str, help='experiments description file')
    ...
    parser.add_argument('--batch-size', default=batch_size, type=int, help='total batch size for all GPUs')
    ...
    parser.add_argument('--device', default=auto_device, type=str, help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    ...
    parser.add_argument('--write_trainbatch_tb', action='store_true',
                        help='write train_batch image to tensorboard once an epoch, may slightly slower train speed '
                             'if open')
...
def main(add_help=True, data_path='datasets/Goats/Goats.yaml', conf_file='configs/yolov6n_finetune.py',
         name='exp', epochs=300, batch_size=8, workers=2, output_dir="runs/detect"):

    """main function of training"""
    args = get_args_parser(add_help=add_help, data_path=data_path, conf_file=conf_file,
                           name=name, epochs=epochs, batch_size=batch_size, workers=workers,
                           output_dir=output_dir).parse_args()
