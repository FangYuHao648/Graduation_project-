...
class RecMainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None, *args, **kwargs):
        ...
        qss_path = abs_path("themes/main_white_trans_A.qss", path_type="current")
        ...
        self.CAM_NUM = 1  # 默认摄像头标号
        self.cameraHandler = MediaHandler(device=self.CAM_NUM, fps=30)  # 摄像头处理器，帧率30
        self.videoHandler = MediaHandler(fps=50)  # 视频处理器，帧率30
        ...
        self.count_name = ["条形码", "二维码"]  # 定义类名列表
        ...
        # 加载模型
        self.model = YOLOv6Detector()  # 创建YOLOv6Detector模型实例
        # 加载训练的模型权重
        self.model.load_model(model_path=abs_path("weights/best_ckpt.pt", path_type="current"))
